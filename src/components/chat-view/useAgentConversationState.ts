...
/**
 * `useSyncExternalStore` 适配器：订阅单个会话在 `AgentSessionService`（插件级
 * 单例，权威状态源）中的状态，不在 React 侧维护 `useState` 镜像。
 *
 * `AgentSessionService.getState`/`subscribe` 在每次调用/发布时都会克隆出一个新的
 ...
 * AgentSessionService publish）时才刷新缓存。这样同一渲染 epoch 内多次调用
 * `getSnapshot` 返回同一引用，满足 `useSyncExternalStore` 的稳定性要求。
 */
export function useAgentConversationState(
  agentService: Pick<AgentSessionService, 'subscribe' | 'getState'>,
